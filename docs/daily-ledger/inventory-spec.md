...
This document describes the schema, formulas, feature flags, audit events, and HTTP endpoints introduced in the Phase A–F inventory-spec wave (module version `1.1.0`).
...
| --- | --- | --- |
| `formal_delivery_workflow_enabled` | `"0"` | Toggles the Phase B formal delivery + receiving workflow. When OFF, deliveries fall back to the legacy commissary push. |
...
## 2. New Tables (11)
Migrations `022`–`029` under `modules/daily-ledger/database/migrations/`.
...
`dl_cashier_withdrawals` gains a `reason_code` column (Phase F).
...
* `deliveries?branch_id=&status=` — list deliveries.
