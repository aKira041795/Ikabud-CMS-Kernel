...
SET @_m011_alt1 := IF(@_m011_req = 0, 'ALTER TABLE kernel_integration_logs ADD COLUMN request_id VARCHAR(64) DEFAULT NULL AFTER error_message', 'SELECT 1');
...
SET @_m011_alt2 := IF(@_m011_corr = 0, 'ALTER TABLE kernel_integration_logs ADD COLUMN correlation_id VARCHAR(64) DEFAULT NULL AFTER request_id', 'SELECT 1');
...
SET @_m011_alt3 := IF(@_m011_dur = 0, 'ALTER TABLE kernel_integration_logs ADD COLUMN duration_ms INT UNSIGNED DEFAULT NULL AFTER correlation_id', 'SELECT 1');
...
SET @_m011_alt4 := IF(@_m011_idx = 0, 'ALTER TABLE kernel_integration_logs ADD KEY idx_kernel_integration_logs_created (created_at)', 'SELECT 1');
