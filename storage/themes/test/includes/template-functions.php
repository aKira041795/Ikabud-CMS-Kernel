 ...
 */
function test_get_option($key, $default = '') {
    return get_theme_mod('test_' . $key, $default);
}

/**
 * Get post data formatted for DiSyL
 */
function test_get_post_data($post = null) {
    if (!$post) {
        $post = get_post();
    }
 ...
 */
function test_get_posts($args = array()) {
    $defaults = array(
        'post_type' => 'post',
        'posts_per_page' => 10,
        'post_status' => 'publish',
