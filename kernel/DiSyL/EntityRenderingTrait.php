        ...
        foreach ($actions as $action) {
            ...
            if ($method === 'post') {
                ...
                $html .= "<form id=\"{$formId}\" method=\"post\" action=\"{$href}\" class=\"inline\"{$onSubmit}>"
                      ...
                      . "</form>"
                      . "<script>(function(){var f=document.getElementById('{$formId}');f.addEventListener('submit',function(e){e.preventDefault();var d=new FormData(f);console.log('[ikb] POST',f.action,Object.fromEntries(d));fetch(f.action,{method:'POST',body:d}).then(function(r){console.log('[ikb] response',r.status,r.statusText);return r.json().catch(function(){return{ok:false,error:'Invalid response'}})}).then(function(j){console.log('[ikb] result',j);if(j.ok&&j.message){var u=new URL(window.location.href);u.searchParams.set('success',j.message);window.location.href=u.toString()}else if(j.error){var u2=new URL(window.location.href);u2.searchParams.set('error',j.error);window.location.href=u2.toString()}else{window.location.reload()}}).catch(function(e){console.warn('[ikb] fetch error',e);window.location.reload()})})})();</script>";
            } else {
