...
function startLanguageServer(context) {
    ...
    const client = new node_1.LanguageClient('disylLanguageServer', 'DiSyL Language Server', serverOptions, clientOptions);
    client.start();
    return client;
}
function registerCommands(context) {
    // Command: Format DiSyL Document
    const formatCommand = vscode.commands.registerCommand('disyl.formatDocument', () => {
    ...
    const previewCommand = vscode.commands.registerCommand('disyl.showPreview', () => {
        const panel = vscode.window.createWebviewPanel('disylPreview', 'DiSyL Component Preview', vscode.ViewColumn.Beside, {
            enableScripts: true,
    ...
    const validateCommand = vscode.commands.registerCommand('disyl.validateDocument', async () => {
        const editor = vscode.window.activeTextEditor;
        if (editor && editor.document.languageId === 'disyl') {
    ...
    const insertComponentCommand = vscode.commands.registerCommand('disyl.insertComponent', async () => {
        const components = [
            'ikb_section',
            'ikb_container',
            'ikb_text',
    ...
    context.subscriptions.push(formatCommand, previewCommand, validateCommand, insertComponentCommand);
}
