...
window.PalRoutes = (function () {
  ...
  return {
    /** Get an API URL by semantic action name. */
    action: function (name, id) {
      var tmpl = api[name];
      if (!tmpl) { console.warn('PalRoutes: unknown action "' + name + '"'); return '#'; }
      return resolve(tmpl, id);
    },
    /** Get an admin page URL by semantic page name. */
    page: function (name, id) {
      var tmpl = pages[name];
      if (!tmpl) { console.warn('PalRoutes: unknown page "' + name + '"'); return '#'; }
      return resolve(tmpl, id);
    }
  };
})();
