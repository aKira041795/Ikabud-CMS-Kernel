...
function aiOpenAiModel(): string
{
    ...
    if ($tier === 'custom') {
        $custom = trim((string)($s['openai_model'] ?? ''));
        if ($custom !== '') {
            return $custom;
        }
    }
    ...
    if ($tier === 'paid') {
        $paid = trim((string)($s['openai_model_paid'] ?? ''));
        if ($paid !== '') {
            return $paid;
        }
    }
    ...
    // default: free
    $free = trim((string)($s['openai_model_free'] ?? ''));
    if ($free !== '') {
        return $free;
    }
                ...
                'suggestions' => [
                    ...
                    'items' => [
                        ...
                        'properties' => [
                            ...
                            'max_per_minute' => ['type' => ['integer', 'null']],
...
function aiOpenAiHttp(string $url, array $payload, string $apiKey, int $timeoutSeconds = 25): array
{
    $ch = curl_init($url);
    if ($ch === false) {
    ...
    if ($body === false) {
        return ['ok' => false, 'error' => 'OpenAI request failed: ' . $err];
    }

    $decoded = json_decode($body, true);
    if (!is_array($decoded)) {
        return ['ok' => false, 'error' => 'OpenAI returned non-JSON response', 'http_code' => $code];
    }
