...
function aiGroqTextGenerate(array $messages, float $temperature = 0.2, bool $json = false, int $timeoutSeconds = 5): array
{
    $apiKey = aiGroqApiKey();
    if ($apiKey === '') {
        return ['ok' => false, 'error' => 'GROQ_API_KEY is not configured'];
    ...
    $resp = aiGroqHttp('https://api.groq.com/openai/v1/chat/completions', $payload, $apiKey);
    if (empty($resp['ok'])) {
        return $resp;
    }
