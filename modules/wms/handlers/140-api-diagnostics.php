...
function wmsApiDiagnosticsTrace(): never
{
    wmsRequireRole(['admin', 'supervisor']);
    
    $productId = (int)wmsInput('product_id', 0);
    if ($productId <= 0) wmsJsonError('product_id is required');
    ...
    $trace = $db->fetchAll(
        'SELECT 
            ...
            p.sku AS product_sku, l.code AS location_code, b.batch_code
...
function wmsApiDiagnosticsReservations(): never
{
    wmsRequireRole(['admin', 'supervisor']);
    ...
    wmsJson(['reservations' => $reservations]);
}
