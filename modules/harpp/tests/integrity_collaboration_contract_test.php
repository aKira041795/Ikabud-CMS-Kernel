...
$transitionMatrix=['CREATED'=>['PENDING','CANCELLED'],'PENDING'=>['NOTIFIED','EXPIRED','SUPERSEDED','CANCELLED'],'NOTIFIED'=>['VIEWED','EXPIRED','SUPERSEDED','CANCELLED'],'VIEWED'=>['DECIDED','EXPIRED','SUPERSEDED','CANCELLED'],'DECIDED'=>['ACKNOWLEDGED','SUPERSEDED','CANCELLED'],'ACKNOWLEDGED'=>['APPLIED','SUPERSEDED','CANCELLED'],'APPLIED'=>['CLOSED'],'CLOSED'=>[],'EXPIRED'=>[],'SUPERSEDED'=>[],'CANCELLED'=>[]];foreach(array_keys($transitionMatrix)as$from)foreach(array_keys($transitionMatrix)as$to)$assert(HarppDecisionService::isTransitionAllowed($from,$to)===in_array($to,$transitionMatrix[$from],true),"transition matrix $from -> $to");
...
$assert(str_contains($decision,"in_array(\$from,['ACKNOWLEDGED','APPLIED'],true)"),'applyAndClose strict source state');
...
$assert(str_contains($detailJs,"['ACKNOWLEDGED', 'APPLIED'].includes(state)"),'apply-and-close UI visible only for acknowledged/applied');
