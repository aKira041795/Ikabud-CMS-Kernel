...
foreach([$root.'/storage/logs/app.log',$root.'/storage/logs/error.log'] as $log){if(is_file($log))file_put_contents($log,'');}
...
try{
 ...
 $push=new HarppPushService($db);$p256dh=rtrim(strtr(base64_encode(random_bytes(65)),'+/','-_'),'=');$auth=rtrim(strtr(base64_encode(random_bytes(16)),'+/','-_'),'=');$saved=$push->subscribe($actor,['endpoint'=>$endpoint,'keys'=>['p256dh'=>$p256dh,'auth'=>$auth],'expirationTime'=>(time()+3600)*1000],$tenantId);$request=$push->buildRequest($endpoint,1700000000);
 ...
 $wellFormed=!empty($saved['ok'])&&count($parts)===3&&($header['alg']??'')==='ES256'&&($claims['aud']??'')==='https://example.com'&&strlen($signature)===64&&strlen($publicKey)===65&&$publicKey[0]==="\x04"&&$request['body']===''&&in_array('Content-Length: 0',$request['headers'],true);$assert('push subscription and empty-payload standards VAPID request',$wellFormed,json_encode(['saved'=>$saved->toArray(),'parts'=>count($parts),'header'=>$header,'claims'=>$claims,'sig'=>strlen($signature),'public'=>strlen($publicKey)]));
