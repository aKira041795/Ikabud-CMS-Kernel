...
final class HarppMessagingService
{
    ...
    public function sendMessage(array $actor,int $conversationId,array $input,?int $tenantId=null)
    {
        ...
        try {
            ...
            foreach($recipients as $recipient){$notice=($this->notifications??=new HarppNotificationService($this->db()))->create($recipient,'message',['event'=>'message.created','conversation_id'=>$conversationId,'message_id'=>$messageId,'sender_type'=>$sender,'in_app_visible'=>$sender==='user'],null,$conversationId,$messageId,false);if(empty($notice['ok']))throw new \RuntimeException((string)$notice['error']);if(empty($notice['data']['idempotent_replay']))$notifications[]=['id'=>(int)$notice['data']['notification_id'],'user_id'=>$recipient];}
            $event=$this->effects('harpp.message.sent','message.sent',$actor,'harpp_message',$messageId,null,['conversation_id'=>$conversationId,'sender_type'=>$sender,'notification_deliveries'=>$notifications]);$this->db()->commit();$this->audit('message.sent',$actor,['conversation_id'=>$conversationId,'message_id'=>$messageId,'sender_type'=>$sender]);
