...
final class HarppAdrService
{
    ...
    public function record(array $actor,array $input,?int $tenantId=null)
    {
        ...
        try{
            ...
            $this->db()->beginTransaction();$stmt=$this->db()->prepare('INSERT INTO harpp_adrs (adr_key,title,context,body,decision,rationale,decision_ref,decided_by,created_at,decided_at) VALUES (:key,:title,:context,:body,:decision,:rationale,:ref,:user,NOW(),:decided)');$stmt->execute([':key'=>$key,':title'=>$source['title'],':context'=>$context,':body'=>$source['body'],':decision'=>$decision,':rationale'=>$rationale,':ref'=>$decisionId,':user'=>$decidedBy,':decided'=>date('Y-m-d H:i:s',$timestamp)]);$id=(int)$this->db()->lastInsertId();
            $event=['name'=>'harpp.adr.recorded','payload'=>['adr_id'=>$id,'decision_id'=>$decisionId]];\app()->events()->fire($event['name'],$event['payload'],'harpp');$audit=\app()->cap()->call('kernel.audit.record@1',['module'=>'harpp','action'=>'adr.recorded','entity_type'=>'harpp_adr','entity_id'=>(string)$id,'new_data'=>$event['payload']],['mode'=>'first','caller_module'=>'harpp']);if(!is_array($audit)||empty($audit['ok']))throw new \RuntimeException('Kernel audit recording failed.');$this->db()->commit();
    ...
    public function get(array $actor,int $adrId,?int $tenantId=null)
    {
        ...
        return is_array($row)?HarppServiceResult::success(['adr'=>$row],'',[],'harpp_adr',$adrId):HarppServiceResult::failure('ADR not found.',404);
    }
    ...
    public function list(array $actor,array $filters=[],?int $tenantId=null)
    {
        ...
        $limit=max(1,min(100,(int)($filters['limit']??25)));$offset=max(0,(int)($filters['offset']??0));$sql='SELECT id,adr_key,title,context,body,decision,rationale,decision_ref,decided_by,created_at,decided_at,superseded_by FROM harpp_adrs'.($where?' WHERE '.implode(' AND ',$where):'').' ORDER BY decided_at DESC,id DESC LIMIT '.$limit.' OFFSET '.$offset;$s=$this->db()->prepare($sql);$s->execute($params);return HarppServiceResult::success(['adrs'=>$s->fetchAll(PDO::FETCH_ASSOC),'limit'=>$limit,'offset'=>$offset]);
    }
