document.addEventListener('DOMContentLoaded',()=>{
  ...
  const isOwner=window.HARPP_CURRENT_USER_ROLE==='owner';
  const escapeHtml=value=>String(value??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
  const show=message=>{status.textContent=message||'';};
  function render(users){
    if(!users.length){list.innerHTML='<p class="muted">No active users.</p>';return;}
    list.innerHTML=users.map(user=>{const protectedRole=!isOwner&&user.role!=='member';return `<form class="user-row grid" data-user-id="${Number(user.id)}" style="grid-template-columns:repeat(auto-fit,minmax(150px,1fr));align-items:end;border-top:1px solid var(--line);padding-top:1rem">
      <label>Full name<input name="full_name" maxlength="255" required value="${escapeHtml(user.full_name)}"${protectedRole?' disabled':''}></label>
      <label>Email<input name="email" type="email" required value="${escapeHtml(user.email)}"${protectedRole?' disabled':''}></label>
      <label>Role<select name="role"${protectedRole?' disabled':''}><option value="member"${user.role==='member'?' selected':''}>Member</option>${isOwner?`<option value="admin"${user.role==='admin'?' selected':''}>Admin</option><option value="owner"${user.role==='owner'?' selected':''}>Owner</option>`:''}</select></label>
      <label>Status<select name="is_active"${protectedRole?' disabled':''}><option value="true"${user.is_active?' selected':''}>Active</option><option value="false"${!user.is_active?' selected':''}>Inactive</option></select></label>
      <span><button class="button save-user" type="submit"${protectedRole?' disabled title="Only an owner can manage this account"':''}>Save</button> <button class="button danger delete-user" type="button"${protectedRole||Number(user.id)===Number(window.HARPP_CURRENT_USER_ID)?` disabled title="${protectedRole?'Only an owner can manage this account':'You cannot delete yourself'}"`:''}>Delete</button></span>
    </form>`;}).join('');
  }
