...
function cmsRenderWidget_image(array $props, array $style, array $attrs, string $children, array $node, array $context): string
{
    ...
    if ($linkUrl !== '') {
        ...
        $imgTag = '<div x-data="{ open: false }">'
            . '<div @click="open = true" style="cursor: zoom-in; opacity: 1; transition: opacity 0.2s;" onmouseover="this.style.opacity=\'0.9\'" onmouseout="this.style.opacity=\'1\'">' . $imgTag . '</div>'
            . '<template x-teleport="body">'
            . '<div x-show="open" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; z-index: 99999; background: rgba(0,0,0,0.9); cursor: zoom-out; backdrop-filter: blur(4px);" x-transition.opacity.duration.200ms @click="open = false" @keydown.escape.window="open = false">'
            . '<div style="display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; padding: 2rem; box-sizing: border-box;">'
            . '<button type="button" @click="open = false" style="position: absolute; top: 1rem; right: 1rem; z-index: 100000; padding: 0.5rem; color: rgba(255,255,255,0.7); background: transparent; border: none; cursor: pointer; transition: color 0.2s;" onmouseover="this.style.color=\'white\'" onmouseout="this.style.color=\'rgba(255,255,255,0.7)\'" aria-label="Close">'
            . '<svg style="width: 2rem; height: 2rem;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>'
            . '</button>'
            . '<img src="' . cmsBuilderEsc($src) . '" alt="' . cmsBuilderEsc($alt) . '" @click.stop style="max-width: 100%; max-height: 100%; object-fit: contain; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25); cursor: default; transition: transform 0.2s;" :style="open ? \'transform: scale(1);\' : \'transform: scale(0.95);\'">'
            . '</div>'
            . '</div>'
            . '</template>'
            . '</div>';
