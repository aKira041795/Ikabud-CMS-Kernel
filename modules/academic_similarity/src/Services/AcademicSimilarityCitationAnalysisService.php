...
class AcademicSimilarityCitationAnalysisService
{
     ...
     *
     * Uses cautious statuses defined in AcademicSimilarityEvidenceTaxonomy.
     * The system does NOT assert that attribution is "present and supported"
     * — that requires linking the citation to the specific source and
     * verifying it supports the claim, which this service does not yet perform.
     */
    public function determineAttributionStatus(
        ...
        if (!$hasCitation) {
            return AcademicSimilarityEvidenceTaxonomy::ATTR_CITATION_NOT_DETECTED;
        }
        if ($hasQuotes && $hasCitation && empty($missingReferences)) {
            ...
            return AcademicSimilarityEvidenceTaxonomy::ATTR_CITATION_AND_QUOTATION;
        }
        if ($hasCitation && !empty($missingReferences)) {
            return AcademicSimilarityEvidenceTaxonomy::ATTR_CITATION_UNRESOLVED;
        }

        return AcademicSimilarityEvidenceTaxonomy::ATTR_CITATION_DETECTED;
    }
}
