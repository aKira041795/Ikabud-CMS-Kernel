...
function palRender(string $template, array $context = []): void
{
    ...
    // Build Workbench shell context from handler variables
    $pc = $context['page_content'] ?? '';
    $user = $context['current_user'] ?? [];
    $context['shell_ctx'] = [
        'application_name' => $context['pal_app_name'],
        'logo_url'         => !empty($context['pal_logo_path']) ? '/' . $context['pal_logo_path'] : null,
        'user_display'     => $user['full_name'] ?? '',
        'page_title'       => $context['page_title'] ?? '',
        'navigation_sections' => palNavSections($pc),
        'user_actions'     => [
            ['label' => 'Sign Out', 'url' => '/api/v1/project-audit-ledger/auth/logout'],
        ],
        'mobile_navigation' => palMobileNav($pc),
...
/**
 * Build PAL navigation sections for workbench:app_shell.
 * @return array<int, array{label: string, collapsed_default: bool, items: array}>
 */
function palNavSections(string $pc): array
{
    return [
        [
        ...
        [
            ...
            'items' => [
                ...
                ['label' => 'BOM',            'url' => '/admin/project-audit-ledger/bom',         'icon_key' => '📋', 'is_active' => $pc === 'bom-list' || $pc === 'bom-form'],
            ],
        ],
        [
            ...
            'items' => [
                ...
                ['label' => 'Issuances',        'url' => '/admin/project-audit-ledger/issuances',          'icon_key' => '📤', 'is_active' => $pc === 'issuance-list'],
                ['label' => 'Returns',          'url' => '/admin/project-audit-ledger/issuances/returns',  'icon_key' => '↩',  'is_active' => $pc === 'material-return-list'],
        ...
        [
            ...
            'items' => [
                ['label' => 'Fabrication',   'url' => '/admin/project-audit-ledger/fabrication/allocations', 'icon_key' => '🔧', 'is_active' => $pc === 'fabrication-allocations'],
...
/**
 * Build PAL mobile bottom navigation items.
 * @return array<int, array{label: string, url: string, icon_key: string, is_active: bool}>
 */
function palMobileNav(string $pc): array
{
    return [
