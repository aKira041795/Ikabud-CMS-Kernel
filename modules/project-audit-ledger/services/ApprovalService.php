...
class palApprovalService
{
     ...
     *
     * Uses SELECT FOR UPDATE + compare-and-set to prevent concurrent reviewers
     * from deciding the same approval twice. If the approval was already decided
     * between our read and write, the UPDATE will affect 0 rows and we abort.
     */
    public function decide(int $approvalId, string $decision, string $remarks = ''): void
    {
        ...
        try {
            // 1. Lock the approval row FOR UPDATE with compare-and-set
            $lockStmt = $this->db->prepare(
                "SELECT * FROM pal_approvals WHERE id = :id AND tenant_id = :tid AND decision = 'pending' FOR UPDATE"
            );
            $lockStmt->execute([':id' => $approvalId, ':tid' => $this->tenantId]);
            $approval = $lockStmt->fetch(PDO::FETCH_ASSOC);
            if (!$approval) {
                $this->db->rollBack();
            ...
            // 2. Compare-and-set update — only succeeds if still pending
            $upd = $this->db->prepare(
                "UPDATE pal_approvals SET decision = :dec, reviewer_id = :rv,
                 decision_date = NOW(), remarks = :rm WHERE id = :id AND tenant_id = :tid AND decision = 'pending'"
            );
            $upd->execute([
                ':dec' => $decision, ':rv' => $this->userId,
                ':rm' => $remarks, ':id' => $approvalId, ':tid' => $this->tenantId,
            ]);

            if ($upd->rowCount() === 0) {
                $this->db->rollBack();
                throw new InvalidArgumentException('Approval was already decided by another reviewer.');
            }

            // 3. Update target entity status
            ...
            // 4. Post-approval side effects (stock movements, cost updates)
    ...
    private function processPurchaseApproval(int $purchaseId): void
    {
        ...
        foreach ($items as $item) {
            ...
            $locationId = !empty($item['storage_location_id']) ? (int)$item['storage_location_id'] : null;

            $bStmt = $this->db->prepare(
                "SELECT COALESCE(quantity,0), COALESCE(avg_cost,0) FROM pal_inventory_balances WHERE material_id=:mid AND tenant_id=:tid AND location_key = COALESCE(:loc, 0)"
            );
            $bStmt->execute([':mid' => $mid, ':tid' => $this->tenantId, ':loc' => $locationId]);
            ...
            $mv = $this->db->prepare("INSERT INTO pal_inventory_movements (tenant_id, material_id, movement_type, reference_type, reference_id, location_id, quantity, unit_cost, total_cost, description, created_by) VALUES (:t,:m,'stock_in','purchase',:rid,:loc,:qty,:uc,:tc,:desc,:cb)");
            $mv->execute([':t' => $this->tenantId, ':m' => $mid, ':rid' => $purchaseId, ':loc' => $locationId, ':qty' => $qty, ':uc' => $unitCost, ':tc' => $totalCost, ':desc' => "Stock in from purchase #{$num}", ':cb' => $this->userId]);

            $ib = $this->db->prepare("INSERT INTO pal_inventory_balances (tenant_id, material_id, location_id, quantity, avg_cost) VALUES (:t,:m,:loc,:qty,:ac) ON DUPLICATE KEY UPDATE quantity=quantity+:qty2, avg_cost=:ac2");
            $ib->execute([':t' => $this->tenantId, ':m' => $mid, ':loc' => $locationId, ':qty' => $qty, ':ac' => $na, ':qty2' => $qty, ':ac2' => $na]);
    ...
    private function processIssuanceApproval(int $issuanceId): void
    {
        ...
        foreach ($items->fetchAll(PDO::FETCH_ASSOC) as $item) {
            ...
            // Use location_key for the balance update to handle nullable location_id
            $this->db->prepare("UPDATE pal_inventory_balances SET quantity=quantity-:qty WHERE material_id=:mid AND tenant_id=:tid AND location_key = COALESCE(
                (SELECT location_id FROM pal_inventory_movements WHERE material_id = :mid2 AND reference_type = 'issuance' AND reference_id = :rid2 AND tenant_id = :tid2 ORDER BY id DESC LIMIT 1), 0
            )")->execute([':qty' => $qty, ':mid' => $item['material_id'], ':tid' => $this->tenantId, ':mid2' => $item['material_id'], ':rid2' => $issuanceId, ':tid2' => $this->tenantId]);
        }
