<?php
/**
 * Cms Akira Workflow Module — Scaffold Test
 *
 * Verifies module bootstrap, context availability, and basic contract compliance.
 * Run: php tests/cms_akira_workflow_module_test.php
 */
...
echo "
=== Cms Akira Workflow MODULE TEST ===
...
echo "── Manifest ──
";
$manifestPath = BASE_PATH . '/modules/cms-akira/cms-akira-workflow/module.json';
t('module.json exists', is_file($manifestPath));

$manifest = json_decode((string)file_get_contents($manifestPath), true);
t('module.json is valid JSON', is_array($manifest));
t('module id matches', ($manifest['id'] ?? '') === 'cms-akira-workflow');
...
";
$all = discoverModules();
t('Module discovered by kernel', isset($all['cms-akira-workflow']));
...
── Routes ──
";
$modulePath = BASE_PATH . '/modules/cms-akira/cms-akira-workflow';
...
t('cawCtx function exists', function_exists('cawCtx'));
t('cawDb function exists', function_exists('cawDb'));
t('cawInput function exists', function_exists('cawInput'));
t('cawRender function exists', function_exists('cawRender'));
