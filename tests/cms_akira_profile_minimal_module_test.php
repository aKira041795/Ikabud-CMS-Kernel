<?php
/**
 * Cms Akira Profile Minimal Module — Scaffold Test
 *
 * Verifies module bootstrap, context availability, and basic contract compliance.
 * Run: php tests/cms_akira_profile_minimal_module_test.php
 */
...
echo "
=== Cms Akira Profile Minimal MODULE TEST ===
...
echo "── Manifest ──
";
$manifestPath = BASE_PATH . '/modules/cms-akira/cms-akira-profile-minimal/module.json';
t('module.json exists', is_file($manifestPath));

$manifest = json_decode((string)file_get_contents($manifestPath), true);
t('module.json is valid JSON', is_array($manifest));
t('module id matches', ($manifest['id'] ?? '') === 'cms-akira-profile-minimal');
...
";
$all = discoverModules();
t('Module discovered by kernel', isset($all['cms-akira-profile-minimal']));
...
── Routes ──
";
$modulePath = BASE_PATH . '/modules/cms-akira/cms-akira-profile-minimal';
...
t('capmCtx function exists', function_exists('capmCtx'));
t('capmDb function exists', function_exists('capmDb'));
t('capmInput function exists', function_exists('capmInput'));
t('capmRender function exists', function_exists('capmRender'));
