...
t('native canonical CMS list uses client-facing summary facts', str_contains($nativeCanonicalBlogHtml, 'Blog Snapshot') && str_contains($nativeCanonicalBlogHtml, 'Published') && str_contains($nativeCanonicalBlogHtml, '1 article') && str_contains($nativeCanonicalBlogHtml, 'Latest articles') && !str_contains($nativeCanonicalBlogHtml, '<strong>Mode</strong>') && !str_contains($nativeCanonicalBlogHtml, '<strong>Route</strong>') && !str_contains($nativeCanonicalBlogHtml, 'Catalog Snapshot'), $nativeCanonicalBlogHtml);
...
$superadminHandlersContent = file_get_contents(BASE_PATH . '/src/http/superadmin-handlers.php') ?: '';
...
t('canonical CMS public layout exposes zoom hover hint copy', str_contains($cmsPublicLayoutContent, "hoverHintText: 'Zoom to enlarge'") && str_contains($cmsPublicLayoutContent, 'cms-lightbox-hover-hint'), $cmsPublicLayoutContent);
...
t('native default theme canonical entity view opts hero media into shared lightbox', str_contains($nativeCanonicalEntityViewTemplateContent, 'data-cms-lightbox="true"'), $nativeCanonicalEntityViewTemplateContent);
...
t('entity-commerce-poc canonical entity view opts hero media into shared lightbox', str_contains(file_get_contents(BASE_PATH . '/storage/cms-themes/entity-commerce-poc/public/entity.view.disyl') ?: '', 'data-cms-lightbox="true"'));
...
t('superadmin feature settings page seeds tenant field values from manifest defaults', str_contains($superadminHandlersContent, "array_key_exists(\$key, \$modSettings)") && str_contains($superadminHandlersContent, ": (\$field['default'] ?? '');"), $superadminHandlersContent);
t('superadmin tenant whitelist keeps only explicitly attached tenant modules', str_contains($superadminHandlersContent, 'superadminTenantRelevantModuleMap($allModules, $selectedEntryModule, $selectedTenantId);') && str_contains($superadminHandlersContent, "foreach ((\$cmsSettings['_installed_submodules'] ?? []) as \$moduleId) {") && str_contains($superadminHandlersContent, "\$tenantSettings = readTenantModuleSettingsForTenant((string)\$moduleId, \$selectedTenantId);") && str_contains($superadminHandlersContent, 'if (!empty($tenantDataSettings)) {') && !str_contains($superadminHandlersContent, "isset(\$allModules['anti-spam']) && !empty(\$allModules['anti-spam']['settings_fields'])"), $superadminHandlersContent);
t('superadmin tenant settings render fields for attached disabled modules', str_contains($superadminHandlersContent, 'if ($hasFields && $tenantDbOk) {'), $superadminHandlersContent);
...
t('canonical storefront product render uses client-facing summary facts', str_contains($storefrontCanonicalProductHtml, 'Product Snapshot') && str_contains($storefrontCanonicalProductHtml, '<strong>Type</strong><span>Product</span>') && str_contains($storefrontCanonicalProductHtml, '<strong>Category</strong><span>Bread</span>') && !str_contains($storefrontCanonicalProductHtml, '<strong>Mode</strong>') && !str_contains($storefrontCanonicalProductHtml, '<strong>Route</strong>'), $storefrontCanonicalProductHtml);
