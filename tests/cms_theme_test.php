...
t('superadmin tenant whitelist keeps only explicitly attached tenant modules', str_contains($superadminHandlersContent, "\$subModules = \$cmsSettings['_installed_submodules'] ?? [];") && str_contains($superadminHandlersContent, '$_candidateTenantSettings = readTenantModuleSettingsForTenant($_candidateModId, $selectedTenantId);') && str_contains($superadminHandlersContent, 'if (!empty($_candidateTenantSettings)) {') && !str_contains($superadminHandlersContent, "isset(\$allModules['anti-spam']) && !empty(\$allModules['anti-spam']['settings_fields'])"), $superadminHandlersContent);
