<?php
/**
 * Cms Akira Media Module — Scaffold Test
 *
 * Verifies module bootstrap, context availability, and basic contract compliance.
 * Run: php tests/cms_akira_media_module_test.php
 */
...
echo "── Manifest ──
";
$manifestPath = BASE_PATH . '/modules/cms-akira/cms-akira-media/module.json';
t('module.json exists', is_file($manifestPath));

$manifest = json_decode((string)file_get_contents($manifestPath), true);
t('module.json is valid JSON', is_array($manifest));
t('module id matches', ($manifest['id'] ?? '') === 'cms-akira-media');
...
";
$all = discoverModules();
t('Module discovered by kernel', isset($all['cms-akira-media']));
...
── Routes ──
";
$modulePath = BASE_PATH . '/modules/cms-akira/cms-akira-media';
...
t('camCtx function exists', function_exists('camCtx'));
t('camDb function exists', function_exists('camDb'));
t('camInput function exists', function_exists('camInput'));
t('camRender function exists', function_exists('camRender'));
