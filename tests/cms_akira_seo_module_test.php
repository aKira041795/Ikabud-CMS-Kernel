<?php
/**
 * Cms Akira Seo Module — Scaffold Test
 *
 * Verifies module bootstrap, context availability, and basic contract compliance.
 * Run: php tests/cms_akira_seo_module_test.php
 */
...
echo "── Manifest ──
";
$manifestPath = BASE_PATH . '/modules/cms-akira/cms-akira-seo/module.json';
t('module.json exists', is_file($manifestPath));

$manifest = json_decode((string)file_get_contents($manifestPath), true);
t('module.json is valid JSON', is_array($manifest));
t('module id matches', ($manifest['id'] ?? '') === 'cms-akira-seo');
...
";
$all = discoverModules();
t('Module discovered by kernel', isset($all['cms-akira-seo']));
...
── Routes ──
";
$modulePath = BASE_PATH . '/modules/cms-akira/cms-akira-seo';
...
t('casCtx function exists', function_exists('casCtx'));
t('casDb function exists', function_exists('casDb'));
t('casInput function exists', function_exists('casInput'));
t('casRender function exists', function_exists('casRender'));
