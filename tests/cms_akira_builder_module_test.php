<?php
/**
 * Cms Akira Builder Module — Scaffold Test
 *
 * Verifies module bootstrap, context availability, and basic contract compliance.
 * Run: php tests/cms_akira_builder_module_test.php
 */
...
echo "── Manifest ──
";
$manifestPath = BASE_PATH . '/modules/cms-akira/cms-akira-builder/module.json';
t('module.json exists', is_file($manifestPath));

$manifest = json_decode((string)file_get_contents($manifestPath), true);
t('module.json is valid JSON', is_array($manifest));
t('module id matches', ($manifest['id'] ?? '') === 'cms-akira-builder');
...
";
$all = discoverModules();
t('Module discovered by kernel', isset($all['cms-akira-builder']));
...
── Routes ──
";
$modulePath = BASE_PATH . '/modules/cms-akira/cms-akira-builder';
...
t('cabCtx function exists', function_exists('cabCtx'));
t('cabDb function exists', function_exists('cabDb'));
t('cabInput function exists', function_exists('cabInput'));
t('cabRender function exists', function_exists('cabRender'));
