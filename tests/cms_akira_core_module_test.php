<?php
/**
 * Cms Akira Core Module — Scaffold Test
 *
 * Verifies module bootstrap, context availability, and basic contract compliance.
 * Run: php tests/cms_akira_core_module_test.php
 */
...
echo "── Manifest ──
";
$manifestPath = BASE_PATH . '/modules/cms-akira/cms-akira-core/module.json';
t('module.json exists', is_file($manifestPath));

$manifest = json_decode((string)file_get_contents($manifestPath), true);
t('module.json is valid JSON', is_array($manifest));
t('module id matches', ($manifest['id'] ?? '') === 'cms-akira-core');
...
";
$all = discoverModules();
t('Module discovered by kernel', isset($all['cms-akira-core']));
...
── Routes ──
";
$modulePath = BASE_PATH . '/modules/cms-akira/cms-akira-core';
...
t('cacCtx function exists', function_exists('cacCtx'));
t('cacDb function exists', function_exists('cacDb'));
t('cacInput function exists', function_exists('cacInput'));
t('cacRender function exists', function_exists('cacRender'));
