...
t('wms manifest owns 14 tables', count((array)($manifest['owns_tables'] ?? [])) === 14, (string)count((array)($manifest['owns_tables'] ?? [])));
t('wms manifest has 10 migrations', count((array)($manifest['migrations'] ?? [])) === 10, (string)count((array)($manifest['migrations'] ?? [])));
t('wms manifest does not claim audit_logs ownership', !in_array('audit_logs', (array)($manifest['owns_tables'] ?? []), true));
