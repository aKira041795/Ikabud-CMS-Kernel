<?php
/**
 * Cms Akira Navigation Module — Scaffold Test
 *
 * Verifies module bootstrap, context availability, and basic contract compliance.
 * Run: php tests/cms_akira_navigation_module_test.php
 */
...
echo "
=== Cms Akira Navigation MODULE TEST ===
...
echo "── Manifest ──
";
$manifestPath = BASE_PATH . '/modules/cms-akira/cms-akira-navigation/module.json';
t('module.json exists', is_file($manifestPath));

$manifest = json_decode((string)file_get_contents($manifestPath), true);
t('module.json is valid JSON', is_array($manifest));
t('module id matches', ($manifest['id'] ?? '') === 'cms-akira-navigation');
...
";
$all = discoverModules();
t('Module discovered by kernel', isset($all['cms-akira-navigation']));
...
── Routes ──
";
$modulePath = BASE_PATH . '/modules/cms-akira/cms-akira-navigation';
...
t('canCtx function exists', function_exists('canCtx'));
t('canDb function exists', function_exists('canDb'));
t('canInput function exists', function_exists('canInput'));
t('canRender function exists', function_exists('canRender'));
