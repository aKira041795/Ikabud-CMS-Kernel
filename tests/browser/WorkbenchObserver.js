 ...
 *   });
 *
 *   await observer.captureHttp('http.request', '/api/v1/project-audit-ledger/projects/*/status');
 *
...
class WorkbenchObserver {
     ...
     */
    async captureHttp(stepName, urlPattern) {
        var self = this;
        return await this.page.evaluate(async function (opts) {
            // Can't intercept from evaluate — use Playwright's route or waitForResponse
            return null;
        }, { stepName: stepName, urlPattern: urlPattern });
    }
